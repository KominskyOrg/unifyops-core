resource "aws_db_subnet_group" "unifyops" {
  ...
  subnet_ids = data.remote_state.vpc.outputs.private_subnets
...
resource "aws_security_group" "db" {
  ...
  vpc_id      = data.remote_state.vpc.outputs.vpc_id

  ingress {
    from_port       = 5432
    to_port         = 5432
    protocol        = "tcp"
    security_groups = [data.remote_state.vpc.outputs.ecs_security_group_id]
  }
